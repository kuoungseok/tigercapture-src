...
- Painter UI Design has a left navigator and right inspector that both default
  ...
  Painter Inspect developer snippets are adapter-owned. Web CSS, provider-
  neutral Tiger JSON, SwiftUI, Jetpack Compose, and shared UMG output are
  generated by `app/painter_ui_dev_snippets.py` and exposed through
  `paint.ui.dev.snippet.inspect`. Unsupported target properties must remain
  explicit. SwiftUI/Compose generation is tested, but native compiler and
  device/runtime validation remain pending until those toolchains are present.
