...
- Developer values are adapter-owned. `Web / CSS` is generated by
  `tigerstudio.painter.ui.web_css.v1`, App uses provider-neutral Tiger JSON,
  iOS uses `tigerstudio.painter.ui.swiftui.v1`, Android uses
  `tigerstudio.painter.ui.compose.v1`, and Unreal uses the exact layer emitted
  by the shared Painter UMG adapter. Native snippets are complete component
  skeletons (`SwiftUI View` / Compose `@Composable`), not mislabeled Tiger JSON.
  Unsupported masks, Boolean geometry, mixed text, variable axes, multiple
  paints, and blend behavior remain explicit beside the generated code.
- Variable inspection follows the selected object's artboard mode through the
...
- Native adapter generation is covered by deterministic Python tests and
  244 px compact Inspector captures. This Windows workstation has no
  `swiftc` or `kotlinc`, so platform compilation and device/runtime capture
  remain `not_run`; generated snippets must not be described as compiled apps.
...
- The overlay reuses resolved theme, responsive, constraint, and geometry data
  when only selection changes. A selection-free normalized document
  fingerprint invalidates the cache when object geometry, style, masks,
  components, or other render-affecting content changes, even if an imported
  document incorrectly reuses an ID or revision.
