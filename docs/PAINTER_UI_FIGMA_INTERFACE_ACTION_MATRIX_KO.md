...
| Editable Vector Network | Pen/Vector tool, double-click Vector Edit, node/segment/Bezier handles, transient contextual bar, open/close/join/split | `paint.ui.vector.node.add/update/remove`, `paint.ui.vector.segment.set/split`, `paint.ui.vector.path.closed.set/join` |
