...
- `paint.ui.vector.node.add/update/remove`,
  ...
  `paint.ui.vector.path.closed.set/join` call the same pure mutation services as
  the UI and preserve one-step Undo plus `.tspaint` round-trip semantics.
- Focused tests, all 295 `test_painter_ui_*` tests, the architecture guard, and
  real 1360x900 / 900x650 screenshot QA pass. Evidence is regenerated by
  `tools/qa_painter_ui_vector_network.py`.
- Reverse Path, Outline Stroke, Simplify, and expanded non-destructive Boolean
  authoring remain explicit M1 follow-up work.
