...
| Audio mixer / extracted audio | `tools/qa_ui_renewal_sound_editor.py` | `debugCapture/ui_renewal_sound_editor_cubase_round_1/dock_sound_editor_mixer_action.png` | Real extracted audio plus two imported stems; Mixer tab shows renewed channel strips plus Master strip with custom low-saturation pan/fader controls, peak/clip meter, Mute/Solo, insert slots, send levels, automation R/W, track type badges, mixer snapshots, and action-backed `audio.mixer.state`. |
