; NSIS installer script for GifCam
; Build:   makensis.exe installer.nsi
; Output:  installer_output\GifCam-Setup-1.0.0.exe
;
; User-level install (no admin required):
;   - files to %LOCALAPPDATA%\GifCam
...
!define ABOUTURL        "https://github.com/kuoungseok/gifcam"
...
UninstallIcon           "resources\gifcam.ico"
...
!define MUI_ICON        "resources\gifcam.ico"
!define MUI_UNICON      "resources\gifcam.ico"
!define MUI_ABORTWARNING
!define MUI_FINISHPAGE_RUN "$INSTDIR\GifCam.exe"
...
Section "Install"
    ...
    ; Copy packaged app tree (must match PyInstaller's dist\GifCam output)
    File /r "dist\GifCam\*.*"

    ; Start Menu
    CreateDirectory "$SMPROGRAMS\${APPNAME}"
    CreateShortCut  "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk"       "$INSTDIR\GifCam.exe" "" "$INSTDIR\GifCam.exe"
    CreateShortCut  "$SMPROGRAMS\${APPNAME}\Uninstall ${APPNAME}.lnk" "$INSTDIR\uninstall.exe" "" "$INSTDIR\uninstall.exe"

    ; Desktop shortcut
    CreateShortCut  "$DESKTOP\${APPNAME}.lnk" "$INSTDIR\GifCam.exe" "" "$INSTDIR\GifCam.exe"
    ...
    WriteRegStr   HKCU "${UNINSTKEY}" "DisplayIcon"          "$\"$INSTDIR\GifCam.exe$\""
...
Section "Uninstall"
    ...
    ; Remove install tree (keeps user data in %USERPROFILE%\Videos\GifCam)
    RMDir /r "$INSTDIR"

    ; Clean registry
    DeleteRegKey HKCU "${UNINSTKEY}"
    DeleteRegKey HKCU "Software\${APPNAME}"
    ; Keep user language preference at HKCU\Software\GifCam\GifCam so future
    ; install reuses it; remove explicitly if user wants a clean slate:
    ; DeleteRegKey HKCU "Software\GifCam"
SectionEnd
