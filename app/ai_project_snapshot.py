            ...
            clips.append(
                {
                    ...
                    "connected_parent_track_id": getattr(clip, "connected_parent_track_id", None),
                    "connected_parent_clip_id": getattr(clip, "connected_parent_clip_id", None),
                    "connected_offset_ms": _int(getattr(clip, "connected_offset_ms", 0)),
                    "clip_role": str(getattr(clip, "clip_role", "") or ""),
                    "role_color": str(getattr(clip, "role_color", "") or ""),
                    "audition_group_id": getattr(clip, "audition_group_id", None),
                    "audition_name": str(getattr(clip, "audition_name", "") or ""),
                    "audition_active_take_id": str(getattr(clip, "audition_active_take_id", "") or ""),
                    "audition_take_count": len(list(getattr(clip, "audition_takes", []) or [])),
                    "timecode_ms": getattr(clip, "timecode_ms", None),
                    "waveform_sync_peak_ms": getattr(clip, "waveform_sync_peak_ms", None),
                    "audio_sync_offset_ms": getattr(clip, "audio_sync_offset_ms", None),
                }
            )
        rows.append(
            {
                ...
                "solo": _bool(getattr(track, "solo", False)),
                ...
                "automation_write": _bool(getattr(track, "automation_write", False)),
                "automation_point_count": len(list(getattr(track, "automation_points", []) or [])),
                "clips": clips,
            }
        )
    return rows
