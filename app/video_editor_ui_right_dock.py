...
def build_right_dock_sections(self) -> None:
    ...
    self._workbench_section_host.setMinimumHeight(230)
    ...
    self._workbench_header_widget.setFixedHeight(15)
    ...
    self.workbench_popout_btn.setFixedSize(15, 13)
    ...
    self._workbench_root_layout = getattr(self, "_top_workbench_layout", None)
    if self._workbench_root_layout is not None:
        self._workbench_root_layout.addWidget(self._workbench_section_host, stretch=1)
        self._workbench_root_index = self._workbench_root_layout.indexOf(
            self._workbench_section_host,
        )
        self._right_dock_scroll.setParent(self._top_workbench_slot)
        self._workbench_root_layout.addWidget(self._right_dock_scroll, stretch=0)
        self._right_dock_scroll.show()
    else:
        self._right_dock_layout.addWidget(self._workbench_section_host, stretch=3)
        self._workbench_root_index = self._main_dock_splitter.indexOf(
            self._right_dock_scroll,
        )
    ...
    if self._capcut_feature_enabled("creator_assist"):
        ...
        self._creator_assist_section_host.setProperty("compactClosedHeight", 26)
        ...
        ca_lay.addWidget(
            self._make_collapsible_section_header(
                ...
                start_open=False,
                on_open=self._ensure_creator_assist_panel,
                popout_callback=self._toggle_creator_assist_popout,
            )
        )
        ca_lay.addWidget(self._creator_assist_placeholder, stretch=0)
        self._right_dock_layout.addWidget(self._creator_assist_section_host, stretch=0)
    ...
    self._ai_script_edit_section_host.setProperty("compactClosedHeight", 26)
    ...
    self._render_queue_section_host.setProperty("compactClosedHeight", 34)
    ...
    self._render_queue_panel = RenderQueuePanel(self._render_queue_section_host)
    rq_lay.addWidget(
        self._make_collapsible_section_header(
            "Render Queue",
            "workbench",
    ...
    rq_lay.addWidget(self._render_queue_panel, stretch=0)
    ...
    self._audio_workspace_section_host.setProperty("compactClosedHeight", 34)
    ...
    self._pip_popout_btn.setFixedSize(18, 18)
    ...
    ssh.setSpacing(6)
    # Subtitle section header row: section label + AI subtitle button.
    _sub_hdr_row = QWidget(self._subtitle_section_host)
    ...
    self._subtitle_panel_toggle_btn.setObjectName("SectionDisclosure")
    ...
    self._right_dock_layout.addWidget(self._subtitle_section_host, stretch=1)
