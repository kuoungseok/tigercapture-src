...
def active_2_5d_camera(composition: MotionComposition, time_ms: float) -> dict[str, float] | None:
...
def project_layer_matrix(
    ...
    *,
    ...
    perspective_scale = max(0.08, min(12.0, camera["z"] / distance * fov_zoom))
    ...
    tx = center_x + (tx - center_x + pan_x) * perspective_scale
    ty = center_y + (ty - center_y + pan_y) * perspective_scale
    angle = math.radians(-camera["roll"])
    cosine, sine = math.cos(angle), math.sin(angle)
    projected = (
        (cosine * a - sine * b) * perspective_scale,
        (sine * a + cosine * b) * perspective_scale,
        (cosine * c - sine * d) * perspective_scale,
        (sine * c + cosine * d) * perspective_scale,
        tx,
        ty,
    )
    return projected
...
def evaluate_replicator(config: Any, time_ms: float) -> list[dict[str, float]]:
    ...
    for index in range(count):
        ...
        rows.append({
            "x": offset[0] * index + noise_x * jitter[0],
            "y": offset[1] * index + noise_y * jitter[1],
