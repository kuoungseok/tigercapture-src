...
def _effect_from_behavior(behavior: MotionBehaviorRef) -> str | None:
    ...
    if kind == "fade":
        ...
        if hinted == "appear" and direction == "in" and bool(params.get("instant", False)):
            return "appear"
        return "fade_out" if direction == "out" else "fade_in"
...
def animation_from_motion_layer(
    ...
    *,
    ...
    if effect == "move":
        ...
        if abs(payload["motion_x"]) > 1.0 or abs(payload["motion_y"]) > 1.0:
            return AnimationSpec(), ["motion distance exceeds native PPT range and requires video bake"]
    elif effect == "scale":
        payload["scale"] = float(behavior.params.get("from", payload["scale"]))
        if payload["scale"] < 0.1 or payload["scale"] > 4.0:
            return AnimationSpec(), ["motion scale exceeds native PPT range and requires video bake"]
    normalized = animation_payload(AnimationSpec.from_dict(payload))
    return AnimationSpec.from_dict(normalized), []
