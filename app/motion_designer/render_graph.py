...
def build_render_graph(
    ...
    *,
    ...
    for layer in composition.layers:
        ...
        if not state.active or layer.layer_type in {"group", "null"} or layer.id in consumed_operand_ids:
            continue
...
def _node_image(node: RenderNode) -> QImage | None:
    if node.image is None and node.layer_type != "adjustment" and node.source_layer is not None:
        node.image = render_source(node.source_layer, node.local_time_ms)
    return node.image
