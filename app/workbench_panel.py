...
from app.workbench_vfx_graph import (
    ...
    vfx_node_graph_status_for_track,
    vfx_node_graphs_from_track,
)
from app.workbench_cards import (
    _AudioEvidenceCard,
    _EditPointEvidenceCard,
    _FxStackRail,
    _NodeRow,
...
class WorkbenchPanel(QWidget):
    ...
    def _audio_tracks_for_sound_editor(self, selected_track) -> list:
        ...
        def _append(raw_tracks) -> None:
            for item in list(raw_tracks or []):
                key = id(item)
                if key in seen:
                    continue
                seen.add(key)
        ...
        while widget is not None:
            _append(getattr(widget, "_audio_tracks", None))
            widget = widget.parentWidget()
        window = self.window()
        if window is not None:
            _append(getattr(window, "_audio_tracks", None))
        if not tracks and selected_track is not None:
            tracks.append(selected_track)
        if selected_track is not None and not any(row is selected_track for row in tracks):
