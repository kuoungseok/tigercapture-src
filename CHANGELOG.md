...
- GitHub Actions `.github/workflows/macos.yml` — Apple Silicon runner
  ...
  GitHub Release (`GifCam-<ver>-mac.dmg`) for every `v*-mac*` tag.
...
- **Capture-result z-order**: main window is re-shown *before*
  ...
  being buried behind the main GifCam window.

### Diagnostics

- `logs/gifcam.log` — every session mirrors stderr to this file with
