...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Build the TigerCapture review automation HTML site.")
    ...
    args.run_qa = False
    args.skip_html = False
    args.skip_ppt = True
    args.deck_mode = "summary"
    args.manifest_only = False
    report = generate_review_assets(args)
    print(json.dumps({"ok": report.get("ok"), "html": report.get("outputs", {}).get("html")}, ensure_ascii=False, indent=2))
