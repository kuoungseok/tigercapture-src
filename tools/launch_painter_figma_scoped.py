"""Open Painter UI Designer with a Figma import, scoped to a single named artboard.

Imports the whole file (so component/style resolution stays intact) then
activates the artboard whose name matches ``--frame`` and enters its group
edit scope so everything else dims out of the way.
"""
...
def main() -> int:
    ...
    parser.add_argument(
        "--umg",
        action="store_true",
        help="also open the UMG widget comparison view",
    )
    ...
    if match is None:
        ...
        raise SystemExit(
            f"No artboard named {args.frame!r} found. Available: {names}"
        )
    artboard_id = str(match["id"])
    page_id = str(match.get("page_id") or "")

    if page_id:
        page_result = registry.execute(
            "paint.ui.page.activate", {"page_id": page_id}
        )
        if not page_result.ok:
            raise RuntimeError(page_result.message)
    activate_result = registry.execute(
        "paint.ui.artboard.activate", {"artboard_id": artboard_id}
    )
    if not activate_result.ok:
        raise RuntimeError(activate_result.message)
    scope_result = registry.execute(
        "paint.ui.selection.scope.enter", {"object_id": artboard_id}
    )
    ...
    def _fit_and_open_umg() -> None:
        registry.execute("paint.ui.view.fit", {"mode": "artboard"})
        if args.umg:
            umg_result = registry.execute(
                "paint.ui.umg.widget_view.set", {"visible": True}
            )
            if not umg_result.ok:
                raise RuntimeError(umg_result.message)

    QTimer.singleShot(100, _fit_and_open_umg)
    return app.exec()
