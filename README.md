...
TigerCapture started as a HoneyCam-style GIF / video screen-capture tool. It
...
Download `TigerCapture-Setup-<version>.exe` from the
[Releases page](https://github.com/kuoungseok/tigercapture/releases) and run
it. No administrator rights required; installs to
`%LOCALAPPDATA%\TigerCapture` and registers under **Settings → Apps →
Installed apps** for clean uninstall.

### Option 2 — Portable

Download `TigerCapture-<version>-portable.zip`, unzip anywhere, run
`TigerCapture.exe`. Delete the folder to "uninstall". Captures stay in
`~/Videos/TigerCapture`.
...
[Actions tab](https://github.com/kuoungseok/tigercapture/actions) for the
